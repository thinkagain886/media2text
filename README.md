...
- **OSS 对象键** 采用 **`SHA-256(原始媒体文件) 前 8 位十六进制 + 下划线 + 清洗后文件名`**（FFmpeg/转写**之前**对上传原文件分块算哈希），在**同一分类**下实现**同内容去重、覆盖幂等**；键示例：`{分类}/audios/a1b2c3d4_产品会议.mp3`、`{分类}/captions/a1b2c3d4_产品会议_transcript.md`。上传前会打日志：哈希、目标 Key、若对象已存在则标注**覆盖上传**。详细说明见 **§13.4**。
- 历史记录表 `records` 增加 **`recognition_type`**。SQLite 启动时自动 `ALTER` 加列；Supabase 老库需执行 `docs/migrations/20260425_add_recognition_type.sql` 或已更新的 `docs/supabase_records.sql`。
...
- **哈希对象**：对用户**上传的原始文件**（视频即视频文件，音频即音频文件）在**任何 FFmpeg 转码或转写之前**计算 **SHA-256**，取**前 8 个十六进制字符**（`content_hash8`），采用**分块读取**，大文件不一次性读入内存。
- **可读名段**：**清洗后的主名**经安全化（去除路径危险字符等）后作为显示名，与 `title` / `source_files[].filename` 的展示语义一致。
- **Key 模式**（`{分类}` 为当前分类名）  
  - 音频：`{分类}/audios/{content_hash8}_{清洗后主名}.mp3`  
  - 转写稿：`{分类}/captions/{content_hash8}_{清洗后主名}_transcript.md`  
- **去重与覆盖**：**相同原始字节内容** → 相同 `content_hash8`；若与当前清洗主名组合后的 Key 一致，则多次上传**覆盖**同一对象，不堆积重复文件。上传前日志会包含：计算的哈希、最终 Key、通过 Head 对象判断是否**已存在**（存在则**覆盖上传**）。
